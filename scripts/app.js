  ...
  */
  var App = {
    ...
    */ 
    ...
    /**
    * Close the dictionary opened from the dictionary.
    * @param e:
    */
...
/**************************** CONFIGURAÇÕES E SALVAMENTO EM LOCAL STORAGE ****************************************/
    ...
    dictionarySearch: function(e) {
      ...
      if(word != this.utilities.string.empty) {
        console.log(word);
        var result = this._getDef(word);
        console.log(result);
        
        if(result.entry == undefined){
          if(result.superEntry == undefined){
            $("#dictionary-result").html("<h2>Definição não encontrada!</h2>");
          }else{
            $("#dictionary-result").html("<h2>"+result.superEntry[0].entry.sense[0].def+"</h2>");
          }
        }else{
          $("#dictionary-result").html("<h2>"+result.entry.sense[0].def+"</h2>");
        }
      }
    },
    ...
    _getDef: function(param) {
      try{
        var request = new XMLHttpRequest();

        request.open("get", "http://dicionario-aberto.net/search-json/"+param, false);
        request.send();

        var fixedResponse = request.responseText.replace(/\\'/g, "'");
